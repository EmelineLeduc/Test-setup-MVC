...
moviesRouter.get("/", (req, res) => {
  const { max_duration, color } = req.body;
  Movie.findAll({ filters: { max_duration, color } })
    .then((movies) => {
      res.json(movies);
    })
    .catch((err) => {
...
moviesRouter.put("/:id", (req, res) => {
  ...
  Movie.findOne(req.params.id)
    .then((movie) => {
      existingMovie = movie;
      if (!existingMovie[0]) return Promise.reject("RECORD_NOT_FOUND");
...
moviesRouter.delete("/:id", (req, res) => {
  Movie.deleteOne(req.params.id)
    .then((deletedMovie) => {
      if (deletedMovie) res.status(200).send("Movie deleted!");
      else res.status(404).send("Movie not found");
    })
    .catch((err) => {
      console.log(err);
